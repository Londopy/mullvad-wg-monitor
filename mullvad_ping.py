...
def _s(city, *ranges):
    ...
    out = []
    for lo, hi in ranges:
        out += [f"us-{city}-wg-{str(n).zfill(3)}.mullvad.net" for n in range(lo, hi+1)]
    return out
...
class MullvadApp(tk.Tk):
    ...
    def _build_header(self):
        ...
        tk.Button(h, text="⌥  github.com/Londopy",
                  font=("Segoe UI",9), fg=T["fg_dim"], bg=T["bg3"],
                  relief=tk.FLAT, padx=10, pady=4, cursor="hand2",
                  command=lambda: webbrowser.open("https://github.com/Londopy")
